    ...
    : QWidget{parent}
{
    ...
    connect(m_pTimer, SIGNAL(timeout()),this, SLOT(uploadFileData()));// 时间间隔之后再上传文件，防止粘包
    ...
    connect(m_pMoveDesDirDB, SIGNAL(clicked(bool)),this, SLOT(moveDesDir()));
...
void fileSystem::createDir()
{
    ...
    QString strCurPath = NetDiskTcpClient::getInstance().getStrCurPath();
...
{
    QString strCurPath = NetDiskTcpClient::getInstance().getStrCurPath();
    QString strRootPath = NetDiskTcpClient::getInstance().getStrRootPath();
    ...
    int index = strCurPath.lastIndexOf("/");        //找到strCurPath中最后一个'/'的位置
    strCurPath = strCurPath.remove(index, strCurPath.size() - index);       //更新strCurPath，使其变为上一目录
...
{
    QString strCurPath = NetDiskTcpClient::getInstance().getStrCurPath(); // 当前目录
    m_strUploadFilePath = QFileDialog::getOpenFileName(); // 将上传的文件的路径保存
...
void fileSystem::uploadFileData()
{
    ...
    while(true)
    {
        ...
        {
            NetDiskTcpClient::getInstance().getTcpSocket().write(pBuffer, iActualSize);
        }
...
void fileSystem::downloadFile()
{
    ...
    QString strCurPath = NetDiskTcpClient::getInstance().getStrCurPath(); // 当前路径
...
void fileSystem::moveFile()
{
    ...
    m_strMoveOldDir = NetDiskTcpClient::getInstance().getStrCurPath(); // 设置移动文件的原目录
...
{
    QString strDesDir = NetDiskTcpClient::getInstance().getStrCurPath(); // 设置移动文件的目标目录

    QMessageBox::StandardButton sbMoveAffirm; // 确认弹框返回值

    QString strMoveAffirm = QString("您确认将 %1 的 %2 文件\n移动到 %3 目录下吗？").arg(m_strMoveOldDir).arg(m_strMoveFileName).arg(strDesDir);
...
void fileSystem::shareFile()
{
    ...
    m_strSharedFilePath = QString("%1/%2").arg(NetDiskTcpClient::getInstance().getStrCurPath()).arg(m_strSharedFileName);

    qDebug() << "分享文件：" << m_strSharedFilePath;

    // 获得好友列表
    QListWidget *friendLW = operateWidget::getInstance().getFriend()->getFriendLW();
